function Tidal_IBE_Flexure_Corrected(range_path, azimuth_path, phi_path, inc_path, cfg)
...
%% =========================================================
%  1. Read range azimuth heading inc data
% =========================================================
fprintf('=== Loading Data ===\n');

[~, name_ext, ~] = fileparts(range_path);
name = extractBefore(name_ext, cfg.range_suffix);  % cfg.range_suffix = ".range-nmt_filtered"

[range_data, R]    = readgeoraster(range_path);
[azimuth_data, ~]  = readgeoraster(azimuth_path);
[lv_phi, ~]        = readgeoraster(phi_path);
[inc, ~]           = readgeoraster(inc_path);
...
t_idx1 = find(date_era5 >= datetime(t1.Year,t1.Month,t1.Day,10,0,0) & ...
              date_era5 <=  datetime(t1.Year,t1.Month,t1.Day,11,0,0));
t_idx2 = find(date_era5 >= datetime(t2.Year,t2.Month,t2.Day,10,0,0) & ...
...
% Diagnostic plot 
figure('Name','IBE Diagnostic','Position',[100 100 1000 400], 'Visible','off');
...
format_fig(gcf, cfg.png, [name, '-IBE_diagnostic']);
...
fprintf('  Grounded ice fraction: %.1f%%\n', ...
        100 * nnz(ground_mask(:)==1) / numel(ground_mask));

figure('Name','BedMachine','Position',[100 100 1600 400], 'Visible','off');
...
format_fig(gcf, cfg.png, [name, '-BedMachine']); 

M = calculate_vaughan_flexure_mask(x, y, thickness, ground_mask, ...
                                   cfg.E, cfg.nu, cfg.rho_sw, cfg.g);

figure('Name','Flexure Mask','Position',[100 100 1800 1600], 'Visible','off');
...
format_fig(gcf, cfg.png, [name, '-FlexureMask']);
...
corrected_range = range_data + tidal_IBE_vertical;

Vgr = corrected_range / time_factor;
...
save_geotiff(cfg.out_vel, [name, '-range.tif'], corrected_range, R, cfg.epsg_code);
...
figure('Name','Velocity Components','Position',[100 100 1500 900], 'Visible','off');
...
format_fig(gcf, cfg.png, [name, '-Velocity']);
