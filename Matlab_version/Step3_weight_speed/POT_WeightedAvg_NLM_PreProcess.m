% =========================================================================
%  方案B：每景影像先 NLM 滤波，再进行加权平均（前处理）
%
%  处理流程:
%    读取 → NLM滤波 → 重估sigma → 加权平均 → 保存
%
%  每组输出 9 个文件（与原始脚本格式完全相同，文件名含 _preNLM 标识）:
%    Vx_preNLM / Vy_preNLM / V_preNLM
%    sigma_Vx_preNLM / sigma_Vy_preNLM / sigma_V_preNLM
%    neff_Vx_preNLM / neff_Vy_preNLM / neff_V_preNLM
%
...
output_dir = '/data2/Phd_Work1/ICE_Velocity_Process/GAMMA_POT_Output2/Cook_80m/Tidal_IBE_Flexure_Correction/Weighted_Average_PreNLM/monthly';
...
% --- NLM 参数 ---
nlm_h        = 50;   % 滤波强度（DegreeOfSmoothing），与噪声水平正相关
...
%% 4. 写分组日志 txt
log_file = fullfile(output_dir, sprintf('group_info_%s_PreNLM.txt', mode));
fid = fopen(log_file, 'w');
fprintf(fid, '方案: B（每景先 NLM 滤波，后加权平均）\n');
fprintf(fid, '时间模式: %s\n', mode);
fprintf(fid, '数据目录: %s\n', data_dir);
fprintf(fid, '总影像对数: %d\n', N);
fprintf(fid, '总分组数: %d\n', G);
...
for g = 1:G
    ...
    for k = 1:n_grp
        ...
        % -------------------------------------------------------
        %  【方案B核心】对每景速度图做 NLM 前处理
        % -------------------------------------------------------
        vx = apply_nlm(vx, nlm_h, nlm_temp_win, nlm_srch_win);
        vy = apply_nlm(vy, nlm_h, nlm_temp_win, nlm_srch_win);
        v  = apply_nlm(v,  nlm_h, nlm_temp_win, nlm_srch_win);

        % --- NLM 后重新估计 sigma（基于平滑后的图像，更稳定）---
        svx = local_std(vx, window_size, ignore_nan, min_valid);
        svy = local_std(vy, window_size, ignore_nan, min_valid);
        sv  = local_std(v,  window_size, ignore_nan, min_valid);

        % --- 重新联合掩膜（NLM可能在边缘引入新NaN）---
        bad_x = isnan(vx) | isnan(svx) | svx < 1e-8;
        bad_y = isnan(vy) | isnan(svy) | svy < 1e-8;
        bad_v = isnan(v)  | isnan(sv)  | sv  < 1e-8;
    ...
    %% 5d. 保存（9 个文件，文件名含 _preNLM 标识）
    geotiffwrite(fullfile(out_sub, [gname '-Vx_preNLM.tif']),       single(Vx_final),  R, 'CoordRefSysCode', epsg);
    geotiffwrite(fullfile(out_sub, [gname '-Vy_preNLM.tif']),       single(Vy_final),  R, 'CoordRefSysCode', epsg);
    geotiffwrite(fullfile(out_sub, [gname '-V_preNLM.tif']),        single(V_final),   R, 'CoordRefSysCode', epsg);
    geotiffwrite(fullfile(out_sub, [gname '-sigma_Vx_preNLM.tif']), single(sVx_final), R, 'CoordRefSysCode', epsg);
    geotiffwrite(fullfile(out_sub, [gname '-sigma_Vy_preNLM.tif']), single(sVy_final), R, 'CoordRefSysCode', epsg);
    geotiffwrite(fullfile(out_sub, [gname '-sigma_V_preNLM.tif']),  single(sV_final),  R, 'CoordRefSysCode', epsg);
    geotiffwrite(fullfile(out_sub, [gname '-neff_Vx_preNLM.tif']),  single(neff_Vx),   R, 'CoordRefSysCode', epsg);
    geotiffwrite(fullfile(out_sub, [gname '-neff_Vy_preNLM.tif']),  single(neff_Vy),   R, 'CoordRefSysCode', epsg);
    geotiffwrite(fullfile(out_sub, [gname '-neff_V_preNLM.tif']),   single(neff_V),    R, 'CoordRefSysCode', epsg);
...
fprintf('输出目录: %s\n', output_dir);
fprintf('\n每组输出 9 个文件:\n');
fprintf('  Vx_preNLM / Vy_preNLM / V_preNLM\n');
fprintf('  sigma_Vx_preNLM / sigma_Vy_preNLM / sigma_V_preNLM\n');
fprintf('  neff_Vx_preNLM  / neff_Vy_preNLM  / neff_V_preNLM\n');
