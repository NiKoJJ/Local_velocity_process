...
out_dir    = fullfile(base_dir, 'POT_Figures2');
...
clim_data  = [-20 20];
clim_pct   = [5 95];       % percentile range, adjust if needed (e.g. [5 95])
hist_bins  = 200;
font_size  = 24;
fig_dpi    = 100;           % dpi for export (high res but fast)
% -------------------------------------------------------------------------
...
for i = 1 : n
    ...
    xt3 = floor(min(rv)/10)*10 : 10 : ceil(max(rv)/10)*10;
    set(ax3, 'FontSize', font_size, 'Box','on', 'XTick', xt3);
    grid on;

    % ---- Tile 4: Azimuth Histogram ----------------------------------------
    ax4 = nexttile;
    if ~isempty(av)
    ...
    xt4 = floor(min(av)/10)*10 : 10 : ceil(max(av)/10)*10;
    set(ax4, 'FontSize', font_size, 'Box','on', 'XTick', xt4);
    grid on;
