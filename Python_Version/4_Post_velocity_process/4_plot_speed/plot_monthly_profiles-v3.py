...
========================
Monthly ice velocity profiles along a transect, with error shading.

Fixes in this version
---------------------
1. Error band: replaced `err <= 0` hard mask with a proper positive-value check;
   error band is drawn independently of the velocity line so partial NaN in
   either does not suppress the other.
2. Smoothing: --smooth option with three methods:
     moving_avg    simple rolling mean (fast, obvious boxcar)
     savgol        Savitzky-Golay (preserves peaks, reduces noise best)
     gaussian      Gaussian kernel (gentler, symmetric)
...
def smooth_profile(arr, method, window):
    ...
    # Linear interpolation over NaN gaps for smooth kernel application
    ...
    if method == "moving_avg":
        ...
        # Edge correction: convolve a ones-array to get the effective kernel size
        norm     = np.convolve(np.ones_like(arr_filled), kernel, mode="same")
        smoothed = smoothed / norm

    elif method == "savgol":
        # polyorder must be < window; use 3 for good noise reduction
        ...
        # sigma ≈ window / 4 gives reasonable roll-off
        sigma    = window / 4.0
...
def _split_segments(mask):
    """
    Split a boolean mask into a list of (start, stop) slice indices
    for each contiguous True run.
    """
    segs = []
    in_seg = False
...
def draw_band_and_line(ax, dist_km, v, err, color, lw=1.3, alpha_fill=0.3,
    ...
    ----------
    nan_policy : str
        "skip"   — draw each contiguous valid segment separately; NaN gaps
                   appear as breaks in the band/line.  Safe, no artefacts.
        "interp" — linearly interpolate over NaN runs shorter than
                   interp_max_gap points before drawing, then fall back
                   to "skip" for longer gaps.
    interp_max_gap : int
        Maximum run of NaN to fill when nan_policy="interp".
    """

    def _interp_short_gaps(arr, max_gap):
        """Fill NaN runs of length <= max_gap by linear interpolation."""
        out   = arr.copy().astype(float)
        valid = np.isfinite(out)
        if valid.sum() < 2:
            return out
        x_all = np.arange(len(out))
        # interp1d baseline (linear between valid points)
        ...
        while i < len(nan_mask):
            if nan_mask[i]:
                ...
                gap = j - i
                if gap <= max_gap:
                    out[i:j] = filled[i:j]   # fill only short gaps
                i = j
            else:
                i += 1
        return out
    ...
    # ── Error band: draw each contiguous segment independently ───────────
    ...
    # ── Velocity line: same segment-by-segment approach ──────────────────
...
def plot_hovmoller(dist_km, ctrl_dist_km, ctrl_labels, profiles,
    ...
    ax1.set_title("Hovmöller diagram — monthly ice velocity", fontsize=12)
...
def main():
    ...
    ap.add_argument("--data_dir", required=True,
                    help="Directory with YYYY-MM_v.tif and YYYY-MM_v_err.tif")
    ...
    ap.add_argument("--n_pts",   type=int,  default=200,
                    help="Sample points per segment (increase for smoother profile)")
    ...
    ap.add_argument("--smooth", default="savgol",
                    ...
                    help=(
                        ...
                        "  none       — raw values (noisy but accurate)\n"
                        "  savgol     — Savitzky-Golay (preserves peaks, recommended)\n"
                        "  moving_avg — boxcar rolling mean (simple)\n"
                        "  gaussian   — Gaussian kernel (gentle roll-off)"))
    ...
    print(f"Found {len(pairs)} monthly pairs ({pairs[0][0]} – {pairs[-1][0]})")
    ...
    for label, v_path, err_path in pairs:
        ...
        # Only mask truly invalid errors: negative, infinite, or exact zero
        # (a very small but positive error is still valid)
