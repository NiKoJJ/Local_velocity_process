...
def _resolve_safe(granule: str) -> str:
    """
    Return path to a Sentinel-1 SAFE/zip file.
    Resolution order:
      1. Current working directory — <granule>.zip
      2. LOCAL_SLC_DIR env variable — <LOCAL_SLC_DIR>/<granule>.zip
      3. Download from ASF (fallback)
    """
    zip_name = f'{granule}.zip'

    cwd_path = Path(zip_name)
    if cwd_path.exists():
        log.info('Using local file (cwd): %s', cwd_path.resolve())
        return str(cwd_path)

    local_dir = os.environ.get('LOCAL_SLC_DIR', '').strip()
    if local_dir:
        local_path = Path(local_dir) / zip_name
        if local_path.exists():
            log.info('Using local file (LOCAL_SLC_DIR): %s', local_path)
            return str(local_path)
        else:
            log.warning('LOCAL_SLC_DIR set but file not found: %s', local_path)

    log.info('Downloading from ASF: %s', granule)
    return download_file(get_download_url(granule), chunk_size=5242880)
...
def process_sentinel1_slc_isce3(
    ...
    safe_ref = _resolve_safe(slc_ref)
    safe_sec = _resolve_safe(slc_sec)
...
def merge_swaths(safe_ref: str, orbit_ref: str, swaths=(1, 2, 3)) -> tuple[int, ...]:
    ...
    for swath in swaths:
        all_ref_bursts = s1reader.load_bursts(safe_ref, orbit_ref, swath, pol)
        ...
        # Only keep burst objects that have a corresponding processed file.
        # When ref and sec are different satellites (e.g. S1A + S1C), the
        # intersection of burst IDs may be smaller than the full safe burst list,
        # so ref_burst_files can have fewer entries than all_ref_bursts.
        # Matching by burst_id string keeps the lists aligned.
        processed_burst_ids = set()
        for bf in ref_burst_files:
            bid = os.path.basename(bf)   # e.g.  t010_021047_iw1
            processed_burst_ids.add(bid)

        ref_bursts = [
            b for b in all_ref_bursts
            if get_isce3_burst_id(b) in processed_burst_ids
        ]

        if len(ref_bursts) != len(ref_burst_files):
            # Fallback: positional alignment (old behaviour, safe when counts match)
            ref_bursts = all_ref_bursts[:len(ref_burst_files)]
